...
def set_user_env(kv_map: dict[str, str]) -> bool:
    """
    设置用户环境变量
    ...
    try:
        ...
        for k, v in kv_map.items():
            winreg.SetValueEx(reg_key, k, 0, winreg.REG_EXPAND_SZ, v)
            print(f"✅ 已成功设置用户环境变量: {k}={v}")
        winreg.CloseKey(reg_key)
...
if __name__ == "__main__":
    ...
    if filename is None:
        filename = os.path.join(
            os.path.dirname(__file__), os.pardir, "config/windows/env.json"
        )

    with open(filename, "r", encoding="utf8") as f:
        env_dict = json.load(f)

    set_user_env(env_dict)
